...
// Matrix element
extern void ml5_0_sloopmatrix_(double p4[4][4], double ans[2][4]);
};
...
pp_hh_noTree::pp_hh_noTree(const std::string& param_card):
    params(new ParametersWrapper(param_card))
{}

std::map<std::pair<int, int>, double> pp_hh_noTree::compute(
...
std::shared_ptr<momemta::MEParameters> pp_hh_noTree::getParameters() {
    return params;
}
