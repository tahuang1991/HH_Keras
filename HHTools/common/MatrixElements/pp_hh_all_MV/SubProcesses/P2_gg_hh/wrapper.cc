...
// Matrix element
extern void ml5_2_sloopmatrix_(double p4[4][4], double ans[2][4]);
};
...
pp_hh_OphiG_ops::pp_hh_OphiG_ops(const std::string& param_card):
    params(new ParametersWrapper(param_card))
{}

std::map<std::pair<int, int>, double> pp_hh_OphiG_ops::compute(
...
std::shared_ptr<momemta::MEParameters> pp_hh_OphiG_ops::getParameters() {
    return params;
}
