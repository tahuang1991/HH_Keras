...
for spin_sig in spins_sig :
    for mass_sig in masses_sig :
        ...
        for suffix in suffixs :
            var = bdtTemplate.replace("DATE", date).replace("SPIN", spin_sig).replace("MASS", mass_sig).replace("SUFFIX", suffix)
            ...
            Rocs.append(roc)
            leg.AddEntry(roc, "X%s %s %s"%(spin_sig, mass_sig, suffix.replace("_8var_bTagMM", "").replace("_"," ")), "P")
